...
export default function Edit({ id }: { id: number | string }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(`${Backend_URL}/sponsor/${id}`, {
        headers: {
      ...
      toast({
        variant: 'destructive',
        description: "Une erreur s'est produite lors de la récupération du sponsor",
        duration: 3000
      });
    }
  }
  ...
  const handleSubmit = async (e:any) => {
    ...
    if(images.length === 0) {
      toast({
        variant: 'destructive',
        description: 'Le sponsor doit avoir un logo est requise',
        duration: 3000
      });
      setLoading(false);
      return;
    ...
    axios.patch(`${Backend_URL}/sponsor/${id}`, {
      name: name,
      website: website,
