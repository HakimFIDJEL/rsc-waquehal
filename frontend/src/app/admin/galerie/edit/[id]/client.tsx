...
export default function Edit({ id } : { id: string|number }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(`${Backend_URL}/galerie/${id}`, {
        headers: {
      ...
      setStatus(galerie.status ? 'online' : 'offline');
     

      const imageUrl = `${Backend_URL}${galerie.image}`;
      const imageResponse = await fetch(imageUrl);
      const blob = await imageResponse.blob();
      const file = new File([blob], 'galerie');
      ...
      toast({
        variant: 'destructive',
        description: "Une erreur s'est produite lors de la récupération de l'image",
        duration: 3000
      });
    }
  }
  ...
  const handleSubmit = async (e:any) => {
    ...
    axios.patch(`${Backend_URL}/galerie/${id}`, {
      title: title,
